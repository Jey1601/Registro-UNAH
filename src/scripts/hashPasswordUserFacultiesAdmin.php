...
$querySelect = "SELECT id_user_faculties_administrator, password_user_faculties_administrator FROM `UsersFacultiesAdministrator`";
$result = $connection->query($querySelect);

if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $id = $row['id_user_faculties_administrator'];
        $password = $row['password_user_faculties_administrator'];
        ...
        if (password_get_info($password)['algo'] == 0) { // Si 'algo' es 0, no está cifrada
            ...
            $queryUpdate = "UPDATE UsersFacultiesAdministrator SET password_user_faculties_administrator = ? WHERE id_user_faculties_administrator = ?";
