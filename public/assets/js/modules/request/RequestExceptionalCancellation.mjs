 ...
 */
  static confirmTerms() {
    ...
    if (!confirmTerms.checked) {
      Alert.display(
        "info",
        "Debe leer terminos y condiciones",
        "Debe leer y aceptar que leyó los terminos y condiciones",
        this.path
      );
      isChecked = false;
    }
 ...
 */
  static async makeRequest(formData) {
    ...
    fetch(
      this.path +
        "api/post/student/makeRequestExceptionalCancellationClasses.php",
      {
        method: "POST",
        body: formData,
      }
  ...
  static addOptionView(tableId) {
    ...
    rows.forEach((row) => {
      ...
      buttonView.addEventListener("click", async () => {
        ...
        this.getDetailsRequestCancellationExceptional(
          parseInt(idSolicitude, 10)
        );
